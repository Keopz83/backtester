...
current_date = get_next_date(df["QUOTE_DATE"].min())
# set end_date to max date in df
end_date = pd.to_datetime(get_next_date(df["QUOTE_DATE"].max()))
...
while True:
    ...
    if quote_close is None:
        ...
        current_date = get_next_date(pd.to_datetime(current_date) + pd.Timedelta(days=30))
